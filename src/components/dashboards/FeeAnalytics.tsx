      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-widest">Fees Collection Over Time</h3>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-widest">Student Fee Breakdown</h3>
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className={`p-2 rounded-xl transition-all border ${showFilters ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-slate-200/60 text-slate-500 hover:bg-slate-50'}`}
              >
                <Filter size={16} />
              </button>
            </div>
            ...
            {showFilters && (
              ...
              >
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 mt-4 border-t border-slate-50">
                  <div>
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5 block">Class</label>
                    <div className="relative">
                      <select 
                        value={selectedClass}
                        onChange={(e) => setSelectedClass(e.target.value)}
                        className="w-full pl-3 pr-8 py-2.5 bg-slate-50 border-none rounded-xl text-xs font-semibold text-slate-700 outline-none appearance-none focus:ring-2 focus:ring-blue-100 transition-all"
                      >
                  ...
                  <div>
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5 block">Session</label>
                    <div className="relative">
                      <select 
                  ...
                  <div>
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5 block">Term</label>
                    <div className="relative">
                      <select 
                        value={selectedTerm}
                  ...
                  <div>
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5 block">Payment Status</label>
                    <div className="relative">
                      <select
                        value={paymentFilter}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50/80 text-[10px] font-semibold text-slate-500 uppercase tracking-widest">
              <tr>
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-2xl bg-slate-100 flex items-center justify-center text-[10px] font-semibold text-slate-500 border border-slate-200 group-hover:scale-110 transition-transform">
                            {formatDisplayString(student.firstName)[0]}{formatDisplayString(student.lastName)[0]}
                          </div>
                          <div className="flex flex-col">
                            <span className="font-semibold text-[13px] text-slate-900 leading-tight tracking-tight uppercase">
                              {formatDisplayString(student.firstName)} {formatDisplayString(student.lastName)}
                            </span>
                            <span className="text-[9px] text-slate-400 uppercase font-semibold tracking-widest">{className}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right font-semibold text-xs text-slate-500">
                        {formatCurrency(totalFee)}
                      </td>
                      <td className="px-6 py-4 text-right font-semibold text-xs text-emerald-600">
                        {formatCurrency(paid)}
                      </td>
                      <td className="px-6 py-4 text-right font-semibold text-[13px] text-slate-900">
                        {formatCurrency(balance)}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5 items-end lg:items-start">
                          <span className={cn(
                            "px-3 py-1 rounded-full text-[8px] font-semibold uppercase tracking-widest border flex items-center gap-1.5",
...
const StatCard = ({ 
  ...
  return (
    <motion.div 
      whileHover={{ y: -2 }}
      className={`p-5 rounded-3xl border ${colorMap[color] || 'bg-white border-slate-200/60'} shadow-sm transition-all`}
    >
      <div className="flex justify-between items-start mb-3">
        ...
        {progress !== undefined && progress > 0 && (
          <span className="text-[10px] font-semibold text-blue-600 bg-blue-100/50 px-2 py-0.5 rounded-full">
            {Math.round(progress)}%
          </span>
        )}
      </div>
