  ...
  return (
    <div className={containerClass}>
      <div className={headerClass}>
        <div className="w-10 h-10 rounded-2xl flex items-center justify-center shadow-lg shrink-0 border bg-gradient-to-br from-blue-600 to-indigo-600 text-white border-blue-400">
          <Sparkles size={20} />
        </div>
        <div>
      ...
      <div className="flex-grow p-4 overflow-y-auto space-y-5">
        ...
        {loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3">
            ...
            <div className="rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-1.5 bg-white border border-slate-200">
              <div className="w-1.5 h-1.5 rounded-full animate-bounce bg-blue-400" style={{ animationDelay: '0ms' }} />
              <div className="w-1.5 h-1.5 rounded-full animate-bounce bg-blue-400" style={{ animationDelay: '150ms' }} />
              <div className="w-1.5 h-1.5 rounded-full animate-bounce bg-blue-400" style={{ animationDelay: '300ms' }} />
            </div>
          </motion.div>
        )}
      ...
      <div className="p-4 border-t bg-white border-slate-100">
        ...
        >
          <input
            ...
            className="flex-grow px-5 py-3 rounded-2xl border border-slate-200 bg-slate-50 text-slate-900 placeholder-slate-400 focus:bg-white focus:border-blue-300 focus:ring-4 focus:ring-blue-500/5 outline-none transition-all text-sm font-medium"
            disabled={loading}
          />
          <button
            type="submit"
            disabled={!input.trim() || loading}
            className="w-11 h-11 rounded-2xl flex items-center justify-center transition-all disabled:opacity-50 shrink-0 shadow-lg bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-blue-600/20"
          >
            <Send size={18} />
          </button>
        </form>
