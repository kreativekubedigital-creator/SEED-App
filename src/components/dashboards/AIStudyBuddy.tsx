...
Refuse to answer any inappropriate, harmful, or unsafe questions politely. Keep your responses concise and engaging.`;
 ...
 return (
 <div className="bg-white backdrop-blur-md rounded-2xl border border-slate-300 shadow-sm overflow-hidden flex flex-col h-[600px]">
 <div className="p-4 border-b border-slate-300 bg-white flex items-center gap-4">
 <div className="w-10 h-10 bg-blue-200 text-slate-900 rounded-2xl flex items-center justify-center shadow-md shrink-0 border border-slate-300">
 <Sparkles size={ 20 } />
 </div>
 <div>
 <h3 className="font-medium text-2xl text-slate-900">AI Study Buddy</h3>
 <p className="text-sm font-medium text-slate-900 mt-1">Safe, guided help for your subjects and homework.</p>
 </div>
 </div>
 ...
 >
 <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${ msg.role  === 'user'?'bg-white backdrop-blur-sm border border-slate-300 text-slate-900 shadow-sm':'bg-blue-600 text-white shadow-sm border border-slate-300'}`}>
 { msg.role  === 'user'? <User size={ 20 } /> : <Bot size={ 20 } />}
 </div>
 <div className={`max-w-[80%] rounded-2xl p-4 ${ msg.role  === 'user'?'bg-blue-600 text-white rounded-tr-sm shadow-md border border-slate-200':'bg-white backdrop-blur-sm border border-slate-300 shadow-sm rounded-tl-sm text-slate-900'}`}>
 { msg.role  === 'user'? (
 <p className="font-medium leading-relaxed">{ msg.text }</p>
 ) : (
 <div className="prose prose-sm max-w-none prose-p:leading-relaxed prose-pre:bg-white prose-pre:border prose-pre:border-slate-300 prose-pre:text-slate-900 prose-a:text-blue-600 prose-strong:text-slate-900">
 ...
 <div className="w-10 h-10 rounded-full bg-blue-200 text-slate-900 flex items-center justify-center shadow-sm shrink-0 border border-slate-300">
 <Bot size={ 20 } />
 </div>
 <div className="bg-white backdrop-blur-sm border border-slate-300 shadow-sm rounded-2xl rounded-tl-sm p-4 flex items-center gap-2">
 ...
 <div className="p-4 bg-white backdrop-blur-md border-t border-slate-300">
 <form
 onSubmit={(e) => {
 e.preventDefault();
 handleSend();
 ...
 className="flex-grow px-6 py-2.5 rounded-full border border-slate-300 bg-white hover:border-white focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-400/20 outline-none transition-all font-medium text-slate-900 placeholder:text-slate-900 cursor-text shadow-sm"
 disabled={ loading }
 />
 <button
 type="submit"
