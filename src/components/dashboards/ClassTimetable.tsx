...
const ClassTimetable: React.FC<ClassTimetableProps> = ({ user, mode, studentClassId, classes: passedClasses }) => {
  ...
  if (loading) return (
    <div className="p-20 flex flex-col items-center justify-center text-slate-900 bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-white/50">
      ...
      <p className="font-medium uppercase tracking-widest text-sm">Loading timetable...</p>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="bg-white/80 backdrop-blur-md p-6 md:p-8 rounded-[2.5rem] border border-white/50 shadow-sm">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            <h2 className="text-3xl font-semibold uppercase tracking-tighter text-slate-900">Class Timetable</h2>
            <p className="text-sm font-medium uppercase tracking-widest text-slate-400 mt-1">Weekly schedule of lessons</p>
          </div>
          ...
          {(mode === 'edit' || (!studentClassId && classes.length > 0)) && (
            <div className="relative group w-full md:w-auto">
              <select
                ...
                className="w-full md:w-auto appearance-none pl-6 pr-12 py-3.5 rounded-2xl border border-slate-200 bg-slate-50 hover:bg-white hover:border-slate-300 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-medium uppercase tracking-widest text-sm text-slate-900 cursor-pointer shadow-sm"
              >
        ...
        {message.text && (
          <div className={`p-4 rounded-2xl mb-8 text-sm font-medium uppercase tracking-widest flex items-center gap-3 border shadow-sm ${
        ...
        {!selectedClass ? (
          <div className="text-center py-20 bg-slate-50/80 rounded-[2.5rem] border-2 border-slate-200/60 border-dashed">
            <p className="text-slate-400 font-medium uppercase tracking-widest text-xs">Please select a class to view its timetable</p>
          </div>
        ) : (
            ...
            <div className="min-w-[1000px] grid grid-cols-5 gap-5 px-6 md:px-0">
              {DAYS.map(day => (
                <div key={day} className="flex flex-col gap-4">
                  <div className="bg-blue-600 py-3 px-4 rounded-2xl text-center font-medium uppercase tracking-widest text-sm text-white shadow-xl shadow-blue-500/10">
                    {day}
                  </div>
                      ...
                      return (
                        <div 
                          ...
                          className={`p-4 rounded-3xl border shadow-sm relative group transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
                            mode === 'edit' ? 'bg-white border-slate-200/60' : style
                          }`}
                        >
                          {mode === 'edit' ? (
                            <div className="space-y-3">
                              <div className="flex items-center gap-2">
                                <input 
                                  ...
                                  className="w-full text-sm font-medium uppercase p-2 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none transition-all"
                                />
                              ...
                              <select 
                                ...
                                className="w-full text-sm font-medium uppercase p-2 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none transition-all cursor-pointer"
                              >
                            ...
                            <>
                              <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-widest opacity-60 mb-2">
                                <Clock size={14} /> 
                                {period.startTime} - {period.endTime}
                              </div>
                              <div className="font-semibold uppercase tracking-tighter text-lg text-slate-900 group-hover:text-blue-500 transition-colors">
                    ...
                    {mode === 'edit' && (
                      <button 
                        ...
                        className="py-4 border-2 border-dashed border-slate-200 rounded-3xl text-slate-400 hover:text-blue-500 hover:border-blue-500 hover:bg-slate-50 transition-all group flex flex-col items-center justify-center gap-1 bg-white/50"
                      >
                        <Plus size={20} className="transition-transform group-hover:rotate-90" />
                        <span className="text-sm font-medium uppercase tracking-widest">Add Period</span>
                      </button>
                    )}
                    
                    {mode === 'view' && (!timetable?.schedule?.[day] || timetable.schedule[day].length === 0) && (
                      <div className="text-center py-10 text-xs font-medium uppercase tracking-widest text-slate-300 italic bg-slate-50/80 rounded-3xl border border-slate-200/60 border-dashed">
                        No Lessons
                      </div>
                    )}
                  </div>
        ...
        {mode === 'edit' && selectedClass && (
          <div className="mt-12 flex justify-end">
            <button
              ...
              disabled={saving}
              className="bg-blue-500 text-white px-8 py-4 rounded-2xl font-medium uppercase tracking-widest text-sm hover:bg-blue-600 hover:shadow-2xl hover:shadow-blue-500/20 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50 shadow-xl shadow-blue-500/10"
            >
              {saving ? (
                <>
